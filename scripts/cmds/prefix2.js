...
module.exports = {
	config: {
		name: "prefix",
		version: "1.7",
		author: "NTKhang + Modified by XNIL + Updated by Hasib",
		countDown: 5,
		role: 0,
	...
	onStart: async function ({ message, role, args, commandName, event, threadsData, getLang }) {
		...
		// New image (Cristiano "shhh" picture you uploaded)
		const prefixImage = "https://i.ibb.co/7QvXbNp/ronaldo-shhh.jpg"; 
	...
	onChat: async function ({ event, message }) {
		if (event.body && event.body.toLowerCase() === "prefix") {
			...
			const infoBox = `
╔═════ 𝙽𝙰𝚁𝚄𝚃𝙾 CHATBOT ════╗
🌐 System Prefix  : ${systemPrefix.padEnd(5)}
💬 Group Prefix   : ${groupPrefix.padEnd(5)}
🕒 Time           : ${timePart.padEnd(8)}
📅 Date           : ${datePart.padEnd(10)}
╚══════════════════╝`;

			// Use same Ronaldo image for info box
			const prefixImage = "https://i.ibb.co/7QvXbNp/ronaldo-shhh.jpg"; 
